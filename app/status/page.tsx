...
import { CheckCircle, AlertCircle, XCircle, Clock, TrendingUp, Activity, AlertTriangle } from 'lucide-react';
...
export default function StatusPage() {
  ...
  const incidents: Incident[] = [
    {
      ...
      status: 'resolved',
      severity: 'minor',
      startTime: new Date('2026-01-10T14:30:00'),
      endTime: new Date('2026-01-10T16:45:00'),
    ...
    },
    {
      ...
      status: 'resolved',
      severity: 'major',
      startTime: new Date('2026-01-08T09:15:00'),
      endTime: new Date('2026-01-08T10:30:00'),
  ...
  const maintenanceWindows: MaintenanceWindow[] = [
    {
      ...
      startTime: new Date('2026-01-20T02:00:00'),
      endTime: new Date('2026-01-20T04:00:00'),
    ...
    },
    {
      ...
      startTime: new Date('2026-02-15T03:00:00'),
      endTime: new Date('2026-02-15T05:00:00'),
      affectedServices: ['Web Application', 'API Service'],
    },
  ];

  // Mock uptime data by period
  ...
  const upcomingMaintenance = maintenanceWindows.filter((m) => m.startTime > currentTime).length;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex flex-col items-end gap-2">
              <div className="text-right">
                ...
                <p className="text-lg font-black text-black">{currentTime.toLocaleTimeString()}</p>
              </div>
            </div>
          </div>
        </div>
                ...
                <div key={incident.id} className="border-4 border-black p-6 rounded-lg bg-white">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t-2 border-gray-200">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-black">
                        {incident.endTime
                          ? `${Math.round((incident.endTime.getTime() - incident.startTime.getTime()) / (1000 * 60))}m`
                          : 'Ongoing'}
                      </p>
                    </div>
              ...
              {maintenanceWindows.map((maintenance) => (
                <div
                  ...
                  className={`border-4 border-black p-6 rounded-lg ${
                    maintenance.startTime > currentTime ? 'bg-heirlock-blue' : 'bg-white'
                  }`}
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className="bg-black text-white px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest whitespace-nowrap ml-4">
                      {maintenance.startTime > currentTime ? 'Upcoming' : 'Past'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t-2 border-gray-400">
                    <div>
                      ...
                      <p className="text-sm font-bold text-black">
                        {maintenance.startTime.toLocaleDateString()} {maintenance.startTime.toLocaleTimeString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-gray-600 uppercase tracking-widest">End</p>
                      <p className="text-sm font-bold text-black">
                        {maintenance.endTime.toLocaleDateString()} {maintenance.endTime.toLocaleTimeString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-gray-600 uppercase tracking-widest">Duration</p>
                      <p className="text-sm font-bold text-black">
                        {Math.round((maintenance.endTime.getTime() - maintenance.startTime.getTime()) / (1000 * 60))}m
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-gray-600 uppercase tracking-widest">Affected Services</p>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="border-4 border-black p-8 rounded-lg bg-heirlock-yellow">
              ...
              <button className="w-full px-4 py-3 bg-black text-white font-bold border-3 border-black rounded-lg hover:opacity-90 transition-opacity">
