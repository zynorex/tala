...
const steps = [
  ...
  const steps = [
    {
      title: "Get access",
      detail: "Request API access at support@usetala.in. You will receive staging and production keys scoped to your team.",
    },
    {
      title: "Create a vault",
              ...
              <ol className="list-inside list-decimal space-y-4 text-sm text-gray-800">
                {steps.map((step) => (
                  <li key={step.title} className="border-2 border-black bg-cream p-3 shadow-sm">
                    ...
                    {step.code && (
                      <pre className="mt-2 overflow-x-auto whitespace-pre-wrap border-2 border-black bg-white p-3 text-xs text-gray-800">{step.code}</pre>
                    )}
                  </li>
                ))}
              </ol>
